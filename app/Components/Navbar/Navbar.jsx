  ...
  return (
    <div className="sticky top-0">
      <nav className=" flex items-center justify-between w-full min-h-[16] shadow-md p-4">
        <div className=" border-4 animate-bounce">
          <Link href="/">RCV</Link>
        </div>
        <div className="border-4 hidden md:block ">
          <ul className="flex">
            {NavBarLinks.map((item) => (
              <li
                key={item.id}
                className="w-[10rem] flex justify-around text-md font-mono ">
        ...
        <div
          className="border-4 cursor-pointer z-20 md:hidden"
          onClick={onClick}>
          {!toggle ? <CiMenuFries size={30} /> : <AiOutlineClose size={30} />}
        </div>
      </nav>
      <div
        className={`bg-red-600 w-full fixed top-0 text-xl z-[999999999]" font-mono ${
          toggle
